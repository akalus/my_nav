(function ($) {
  ...
  function switch_mode() {
    if ($("body").hasClass("io-black-mode")) {
      ...
      else $(".switch-dark-mode").attr("title", "日间模式");
      $(".mode-ico").removeClass("icon-night");
      $(".mode-ico").addClass("icon-light");
    } else {
      if ($(".switch-dark-mode").attr("data-original-title"))
        $(".switch-dark-mode").attr("data-original-title", "夜间模式");
      else $(".switch-dark-mode").attr("title", "夜间模式");
  ...
  function stickFooter() {
    ...
    if ($(".main-footer").hasClass("text-xs")) {
      ...
      if (
        win_height >
        main_content_height - parseInt($(".main-footer").css("marginTop"), 10)
      ) {
        $(".main-footer").css({
  ...
  function trigger_resizable(isNoAnim) {
    if (
      (theme.minNav == "1" && !isMin && 767.98 < $(window).width()) ||
      (!isMin && 767.98 < $(window).width() && $(window).width() < 1024)
    ) {
      ...
      theme.minNav != "1" &&
      ((isMin && $(window).width() >= 1024) ||
        (isMobileMin && !isMin && $(window).width() >= 1024))
    ) {
  ...
  $(".sidebar-menu-inner a").on("click", function () {
    //.sidebar-menu-inner a //.has-sub a
  ...
  $(document).on(
    ...
    function () {
      var offset = 2;
      if ($(this).parents(".flex-bottom").length != 0) offset = -3;
      $(".sidebar-popup.second").length == 0 &&
        $("body").append(
      ...
      $.ajax({
        ...
        .done(function (response) {
          if (response.trim()) {
            ...
            box.html(url);
            if (isPC())
              url.find('[data-toggle="tooltip"]').tooltip({ trigger: "hover" });
          } else {
          }
          t.removeClass("disabled");
        })
  ...
  $(document).on("click", ".ajax-list-home a", function (event) {
    ...
    loadAjax(
      $(this),
      $(this).parents(".ajax-list-home"),
      ".ajax-" + $(this).parents(".ajax-list-home").data("id")
    );
  });
      ...
      $.ajax({
        ...
        .done(function (response) {
          if (response.trim()) {
            ...
            if (url)
              t.parents(".d-flex.flex-fill.flex-tab")
  ...
  function addSite(site, isLive, isHeader) {
    ...
    var newSite = $(
      '<div class="url-card  col-6 ' +
        ...
        "</div>" +
        "</a></div>" +
        '<a href="javascript:;" class="text-center remove-site" data-id="' +
        site.id +
        '" style="display: none"><i class="iconfont icon-close-circle"></i></a>' +
        "</div>"
    );
    if (isLive) {
      if (isHeader) $(".my-click-list").prepend(newSite);
      else $(".my-click-list").append(newSite);
      newSite.children(".remove-site").on("click", removeLiveSite);
    } else {
      $("#add-site").before(newSite);
      newSite.children(".remove-site").on("click", removeSite);
    }
    if (isEdit) newSite.children(".remove-site").show();
    if (isPC())
      $('.new-site[data-toggle="tooltip"]').tooltip({ trigger: "hover" });
  }
      ...
      .done(function (response) {
        ...
        var newSite = $(
          '<div id="url-' +
            ...
            "</div>" +
            "</a></div>" +
            '<a href="javascript:;" class="text-center remove-cm-site" data-action="delete_custom_url" data-id="' +
            id +
            '"><i class="iconfont icon-close-circle"></i></a>' +
            "</div>"
        );
  ...
  function ioSortable() {
    if ($(".customize-sites").hasClass("edit")) {
      if (isPC())
        $('.customize-sites .new-site[data-toggle="tooltip"]').tooltip(
          "disable"
        );
  ...
  $(document).on("click", ".url-card a.card", function (event) {
    ...
    if (isNew) {
      ...
      if (liveList.length > maxSite - 1) {
        $(".my-click-list .site-" + liveList[maxSite - 1].id)
          .parent()
          .remove();
  ...
  function intoSearch() {
    if (window.localStorage.getItem("searchlist")) {
      ...
      $(
        ".hide-type-list input#m_" + window.localStorage.getItem("searchlist")
      ).prop("checked", true);
    }
    ...
    $('.hide-type-list input:radio[name="type"]:checked')
      .parents(".search-group")
      .addClass("s-current");
    $('.hide-type-list input:radio[name="type2"]:checked')
      .parents(".search-group")
      .addClass("s-current");
  ...
  $(".hide-type-list .search-group input").on("click", function () {
    ...
    else parent.find(".search-key").attr("zhannei", "");
  ...
  $(".nav-login-user.dropdown").hover(
    function () {
      if (!$(this).hasClass("show")) $(this).children("a").click();
    },
    function () {
...
function toTarget(menu, padding, isMult) {
  ...
  if (target && 0 < target.length) {
    ...
    if (isMult) target = menu.find(".active").parent();
    else target = menu.find(".active");
  }
  if (0 < target.length) {
    if (padding)
      slider.css({
...
function ioPopupTips(type, msg, callBack) {
  ...
  var html =
    ...
    c +
    ' io-bomb-sm io-bomb-open">' +
    '<div class="io-bomb-overlay"></div>' +
    '<div class="io-bomb-body text-center">' +
    '<div class="io-bomb-content bg-white px-5"><i class="iconfont ' +
    ico +
    ' icon-8x"></i>' +
      ...
      $.each(i.children(), function (a, i) {
        ...
        (e += 1),
          ...
          i.tagName.toLowerCase() == "H3".toLowerCase() &&
            ...
            (f = { type: "header", title: m, add_date: n, last_modified: o }),
            (j = !0)),
          "a" == i.tagName.toLowerCase() &&
            ...
            (s = p.attr("add_date")),
            p.attr("icon"),
            (h = { type: "link", title: q, href: r, add_date: s }),
            c.items.push(h));
      });
    });
}
